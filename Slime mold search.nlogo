globals [rule-set p min-step max-step turn-angle]

; Slime mold and oat flakes are breeds of turtles
breed [ slime-mold spore ]
breed [ oats flake ]
turtles-own [ step ]

; Patches obtain units of energy from passing oat flakes
patches-own [ energy ]
...
to go
  ask slime-mold [ run rule-set ]
  ask patches
  [ if any? slime-mold-here
    [ set pcolor yellow]
  ]
  tick
  if not any? slime-mold [
    connect-paths
    ask oats [ make-oats ]
    stop
  ]
...
to forage
  move-check               ; Move the turtle and check if a path is crossed
  rt (random turn-angle)   ; Turn right by random angle up to turn-angle
  hatch 1                  ; "hatch" causes the turtle to duplicate itself
  set color yellow
  move-check
  lt (random (2 * turn-angle))
  move-check
end

; Move turtle one space ahead, check if the turtle is crossing a path of another turtle
to move-check
  set step random ((max-step - min-step) + min-step)
  while [step > 0]
  [
...
to connect-paths
  ; If patch at (100,100) is not yellow, fill in neighbors to complete connected path
  set p 100
  while [ [pcolor] of patch p p != yellow ]
  [
    ask patch p p [set pcolor yellow]
    set p p - 1
  ]
end

to make-oats
  hatch-oats 1 [set color red setxy 100 100]
end

; Written: 23 July 2021 WildPeaches
; See Info tab for full details
; Derived from L-System Fractals.nlogo
@#$#@#$#@
GRAPHICS-WINDOW
281
11
